...
const Figure: React.FC<{ product: Product; fill: string }> = ({ product, fill }) => {
  ...
  return (
    <svg viewBox="0 0 120 120" className="w-28 h-28" aria-hidden="true">
      {product.figure === 'face' && (
        <>
          <ellipse cx="60" cy="60" rx="36" ry="44" fill={cheek ? 'var(--art-skin)' : fill} stroke={traco} strokeWidth="3" />
          {cheek && (
            <>
      ...
      {product.figure === 'nail' && (
        <>
          <path d="M44 108q-6-26 0-52 4-18 16-18t16 18q6 26 0 52Z" fill="var(--art-skin)" stroke={traco} strokeWidth="3" />
      ...
      pinned={(
        <>
          {/* Seletor de modo. As duas metades são coisas diferentes — uma se
            ...
            value={mode}
            options={[{ key: 'products', label: t('mkModeProducts') }, { key: 'tools', label: t('mkModeTools') }]}
            onChange={setMode}
            theme={theme}
          />
          <PhraseCard
            theme={theme}
            phrase={shown}
            alt={showNative ? shownNative : null}
            Listen={Listen}
                    ...
                    {opcoes.map((o) => {
                      ...
                      return (
                        <button
                          ...
                          aria-pressed={active}
                          className={`rounded-2xl border px-3 py-2 text-sm font-bold tap active:scale-95 ${active ? 'border-transparent shadow' : 'bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700'}`}
                          style={active ? { backgroundColor: `${theme.hex}18`, boxShadow: 'inset 0 0 0 2px var(--tema-texto)', color: 'var(--tema-texto)' } : undefined}
                        >
                          {dim.visual && (
                            ...
                            <span
                              className="block w-10 h-6 rounded-lg border border-slate-400 dark:border-slate-500 mx-auto mb-1"
                              style={cor
                                ? { backgroundColor: cor }
                                : { backgroundImage: 'linear-gradient(to top right, transparent 46%, var(--art-line) 46%, var(--art-line) 54%, transparent 54%)' }}
                            />
                          )}
