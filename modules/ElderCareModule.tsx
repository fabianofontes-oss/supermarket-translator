...
function Toggle<T extends string>({ theme, label, hint, value, options, onPick }: {
  ...
  return (
    <section className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 p-4">
      ...
      {hint && <p className="text-sm text-gray-600 dark:text-slate-300 leading-snug mt-2" dir="auto">{hint}</p>}
    </section>
  );
}
    ...
    >
      ...
      <div {...panelProps(tabsId, mode)} className="space-y-4">
        ...
        <section className="rounded-3xl border-2 border-red-600 bg-red-50 dark:bg-red-950 overflow-hidden">
          ...
          >
            <span className="text-base font-extrabold text-red-700 dark:text-red-300 uppercase tracking-wide" dir="auto">{t('ecEmergency')}</span>
            <span className="text-xl font-extrabold text-red-700 dark:text-red-300 tabular-nums">{EMERGENCY_NUMBER}</span>
          </button>
          {sos && (
            <div className="px-4 pb-4">
              <p className="text-sm text-red-900 dark:text-red-300 leading-snug mb-3" dir="auto">{t('ecEmergencyHint')}</p>
        ...
        {mode === 'talk' && (
          <>
            ...
            {action.note && (
              <p className="text-sm text-gray-600 dark:text-slate-300 leading-snug px-1" dir="auto">{action.note[read]}</p>
            )}
        ...
        {mode === 'report' && (
          <>
            ...
            <section>
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500 dark:text-slate-400 mb-2 px-1">{t('ecWhen')}</h2>
          ...
          <>
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                {showNative && <p className="text-sm text-gray-500 dark:text-slate-400" dir="auto">{tool.names[native]}</p>}
                {tool.note && <p className="text-sm text-gray-600 dark:text-slate-300 leading-snug mt-1" dir="auto">{tool.note[read]}</p>}
              </div>
            </div>

            <section>
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500 dark:text-slate-400 mb-2 px-1">{t('ecHowToAsk')}</h2>
              <Pills<ToolFrame>
                theme={theme}
                items={TOOL_FRAMES}
                active={frame.key}
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 leading-snug px-1 pt-2" dir="auto">{t('ecSafety')}</p>
      </div>
    </ModuleShell>
  );
}
