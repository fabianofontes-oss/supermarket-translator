    ...
    <div className="w-full bg-slate-50 text-gray-800 flex flex-col h-[100dvh] relative overflow-hidden font-sans">
      <header className="flex-shrink-0 text-white shadow-lg z-30 rounded-b-3xl" style={{ background: `linear-gradient(to bottom, ${theme.hex}, ${theme.hex}e6)` }}>
        <div className="flex items-center justify-between px-4 pt-4 pb-4 max-w-3xl mx-auto">
          ...
          {/* `ecTitle` e não `moduleElderCare`: o <h1> tem 190px úteis a 375px de
              largura (o resto é o botão de casa de um lado e o par de botões do
              outro), e "CUIDAR DE IDOSOS" mede 213px — saía como "CUIDAR DE ID…",
              que não diz nada. O cartão do hub tem a linha inteira e quebra, então
              lá continua o nome completo. Nenhum outro módulo precisou disto
              porque nenhum outro nome passa de "Supermercado", que mede 186px. */}
          <h1 className="flex-1 mx-2 text-center font-bold text-2xl uppercase tracking-tight truncate">{t('ecTitle')}</h1>
