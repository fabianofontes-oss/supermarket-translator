...
*   Módulo irmão "Limpeza / house cleaner", generativo, reaproveitando o eixo de tratamento. Não refazer os 96 produtos de `supermarket/data/cleaningData.ts`: o espaço livre é verbo de tarefa, cômodo e superfície, agenda e conversa com a patroa. Duas lacunas reais de vocabulário já conferidas — `fregona` e `aspiradora` não existem em lugar nenhum do repositório, e são os dois objetos número um da limpeza espanhola.
